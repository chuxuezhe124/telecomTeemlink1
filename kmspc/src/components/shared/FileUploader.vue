  ...
  <div name="FileUploader" class="file-uploader">
    <div>
      <el-upload
        ...
        >
        <el-button size="mini" v-if="this.params.folderId"><i class="el-icon-upload2"></i> {{$t('file.upload_document')}}</el-button>
        <el-button type="primary" v-else> {{$t('btns.upload_document')}}</el-button>
      </el-upload>
      <el-dialog
        @click.stop.prevent
        :title="$t('file.upload_document')"
          ...
          <div>
            <ul class="el-upload-list el-upload-list--text">
              <li class="el-upload-list__item is-success" v-for="(file, index) in fileList" :key="index">
                ...
                <div>
                  <el-progress :percentage="file.marks" v-show="isShowProgress" style="padding-left:5px"></el-progress>
                  <div style="font-size: 8px;padding-top: 5px;text-align: right;" v-if="file.marks==100 && isShowText">{{$t('placeholder.background_progress_wait')}}</div>
                </div>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleSettingsOk" size="mini" :disabled="disabled">{{$t('btns.determine')}}</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
      ...
      (res) =>{   
       let uploadComplete=0; 
        for(let i=0; i<this.fileList.length; i++) {
          if(res.uid === this.fileList[i].uid) {
            this.fileList[i].marks = res.num;
            this.fileList[i].marksValue= res.complete;
            this.$forceUpdate()
    
          }
          uploadComplete=this.fileList[i].marks===100?(uploadComplete+1):uploadComplete;
          if(uploadComplete===this.fileList.length){
            this.disabled=false;
            this.isShowProgress=false
          }
        }
